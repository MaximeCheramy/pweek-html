...
Pweek.Piece.prototype.drop = function(grid) {
	...
	while (ok) {
		for (var i = 0; i < 2; i++) {
			if (this.coords[i][0] <= 0 || grid[this.coords[i][0] - 1][this.coords[i][1]] > 0) {
				ok = false;
			}
		}
		if (ok) {
...
Pweek.Piece.prototype.moveLeft = function(grid) {
	var ok = true;
	for (var i = 0; i < 2; i++) {
		if (this.coords[i][1] <= 0 || grid[this.coords[i][0]][this.coords[i][1] - 1] > 0) {
			ok = false;
		}
	}
	if (ok) {
...
Pweek.Piece.prototype.moveRight = function(grid) {
	var ok = true;
	for (var i = 0; i < 2; i++) {
		if (this.coords[i][1] >= COLUMNS - 1 || grid[this.coords[i][0]][this.coords[i][1] + 1] > 0) {
			ok = false;
		}
	}
	if (ok) {
	...
	if (this.state == 'move') {
		...
		if (this.sum == delta) {
			...
			for (var i = 0; i < this.removes.length; i++) {
				...
				this.addScore(b[0], b[1], r.length * 10 * (r.length - 3 + this.combo));
			}
		}
		if (this.sum > 300) {
...
Pweek.GameLogic.prototype.floodfill = function(l, c, coul, list) {
	if (l < 0 || c < 0 || l >= LINES || c >= COLUMNS || this.grid[l][c] != coul
			|| this.gridFF[l][c]) {
		return 0;
	}
...
Pweek.GameLogic.prototype.gravity = function() {
	...
	for (var l = 0; l < LINES * 2; l++) {
		for (var c = 0; c < COLUMNS; c++) {
			...
			if (s && s.position.y != y) {
                s.frame = 1;
				Pweek.game.add.tween(s).to({y: y}, 3 * (y - s.position.y), Phaser.Easing.Bounce.Out, true)
