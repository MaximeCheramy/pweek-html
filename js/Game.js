...
Pweek.Game.prototype = {
  create: function() {
	...
	this.scoreText = this.game.add.text(260, 20, '0', {'fill': '#FFFFFF', 'align': 'right'});
	if (this.mode == 'solo') {
		this.levelText = this.game.add.text(40, 270, 'Level 1', {'font': '20px arial', 'fill': '#FFFFFF'});
	}
	...
	this.game.input.onUp.add(function(pointer) {
		...
		if (this.logic.state == 'move' && this.pointer.duration < 150 && dx < 10 && dy < 10 && this.pointer.duration > 5) {
			this.logic.rotateLeft();
		}
		this.pointer = null;
	}, this);
  ...
  updatePositionPiece: function() {
	var c1 = this.convertPosition(this.logic.piece.coords[0][1], this.logic.piece.coords[0][0]);
	var c2 = this.convertPosition(this.logic.piece.coords[1][1], this.logic.piece.coords[1][0]);
	Pweek.game.add.tween(this.white).to({y: c1[1]}, 200, Phaser.Easing.Linear.NONE, true);
	Pweek.game.add.tween(this.white).to({x: c1[0]}, 100, Phaser.Easing.Linear.NONE, true);

	Pweek.game.add.tween(this.s1).to({y: c1[1]}, 200, Phaser.Easing.Linear.NONE, true);
	Pweek.game.add.tween(this.s1).to({x: c1[0]}, 100, Phaser.Easing.Linear.NONE, true);
	Pweek.game.add.tween(this.s2).to({y: c2[1]}, 200, Phaser.Easing.Linear.NONE, true);
	Pweek.game.add.tween(this.s2).to({x: c2[0]}, 100, Phaser.Easing.Linear.NONE, true);
  },
  createPiece: function() {
	this.s1 = this.ns1;
	this.s2 = this.ns2;
	var nc1 = this.positionNextPiece(this.logic.nextPiece.coords[0][1] - COLUMNS / 2,
			 								  this.logic.nextPiece.coords[0][0] - LINES);
	var nc2 = this.positionNextPiece(this.logic.nextPiece.coords[1][1] - COLUMNS / 2,
			 								  this.logic.nextPiece.coords[1][0] - LINES);

	this.ns1 = Pweek.game.add.sprite(nc1[0], nc1[1], spriteName[this.logic.nextPiece.coords[0][2]]);
	this.ns2 = Pweek.game.add.sprite(nc2[0], nc2[1], spriteName[this.logic.nextPiece.coords[1][2]]);
  ...
  removeSprites: function(list) {
	  for (var i = 0; i < list.length; i++) {
			...
			this.game.add.tween(s).to({x: x}, 500, Phaser.Easing.Linear.NONE, true);
			this.game.add.tween(s).to({y : 1000}, 500, Phaser.Easing.Back.In, true);
			this.game.add.tween(s).to({angle: angle}, 500, Phaser.Easing.Cubic.Out, true)
  ...
  addScore: function(x, y, score) {
	...
	this.game.add.tween(t).to({x: this.scoreText.position.x}, 500, Phaser.Easing.Linear.None, true)
		.onComplete.add(function() { this.scoreText.setText(this.logic.score); }, this);

	this.game.add.tween(t).to({y: this.scoreText.position.y}, 500, Phaser.Easing.Exponential.Out, true)
